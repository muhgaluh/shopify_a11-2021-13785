...
# Post Add Product
@product_router.post('products.json', auth=apiAuth, response=ProductIn)
def add_product(request, product_in: ProductIn):
    product = Product.objects.create(**product_in.dict())
    return product_in

# Post Add Product Variant
@product_router.post('products/{product_id}/variants.json', auth=apiAuth, response=ProductVariantIn)
